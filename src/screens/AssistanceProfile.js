...
export default function AssistanceProfile() {
  ...
  const formSubmitHandler = (data) => {
    ...
    // }
    dispatch(editAssistantProfile(formData));
  };
  ...
  useEffect(() => {
    if (isErrorEditAssistantProfile) {
      toast.error(messageEditAssistantProfile);
      setShowModal(true);
    }

    if (isSuccessEditAssistantProfile) {
      toast.success(messageEditAssistantProfile);
      setShowModal(false);
      reset();
    }
    if (messageEditAssistantProfile) {
      dispatch(getAssistantProfile(user._id));
    }
