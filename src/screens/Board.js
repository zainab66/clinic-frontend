...
const KanbanBoard = () => {
  ...
  const handleDragEnd = async ({ draggableId, source, destination }) => {
    ...
    // fetch(`http://localhost:3001/api/task/${card._id}`, {
    //   method: 'PATCH',
  ...
  useEffect(() => {
    if (isErrorDelTask) {
      toast.error(messageDelTask);
      setShowModal(true);
    }

    if (isSuccessDelTask) {
      toast.success(messageDelTask);
      setShowModal(false);
    }
    if (messageDelTask) {
      dispatch(getLists());
  ...
  }, [isErrorDelTask, isSuccessDelTask, messageDelTask, dispatch]);

  useEffect(() => {
    if (isErrorAddNewTask) {
      toast.error(messageAddNewTask);
      setShowModal(true);
    }

    if (isSuccessAddNewTask) {
      toast.success(messageAddNewTask);
      setShowModal(false);
      reset();
    }
