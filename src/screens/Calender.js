...
export default function Calender() {
  ...
  useEffect(() => {
    if (isErrorAddNewEvent) {
      toast.error(messageAddNewEvent);
      setShowModal(true);
    }

    if (isSuccessAddNewEvent) {
      toast.success(messageAddNewEvent);
      setShowModal(false);
      reset();
    }
  ...
  useEffect(() => {
    if (isErrorDelEvent) {
      toast.error(messageDelEvent);
      setShowModalToChange(true);
    }

    if (isSuccessDelEvent) {
      toast.success(messageDelEvent);
      setShowModalToChange(false);
    }
    if (messageDelEvent) {
      dispatch(getEvents());
    ...
    dispatch(resetReducerEvent());
  }, [isErrorDelEvent, isSuccessDelEvent, dispatch, messageDelEvent]);

  useEffect(() => {
    if (isErrorEditEvent) {
      toast.error(messageEditEvent);
      setShowUpdateModal(true);
    }

    if (isSuccessEditEvent) {
      toast.success(messageEditEvent);
    ...
    dispatch(resetReducerEvent());
  }, [isErrorEditEvent, isSuccessEditEvent, dispatch, messageEditEvent]);
                ...
                <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-gray-50 outline-none focus:outline-none ">
                  ...
                  <div className=" px-8 pt-6 pb-8 w-full">
                    ...
                    <div className="flex items-start flex-wrap justify-between ">
                      {' '}
                      <label className="block text-black text-sm  mb-1 pt-1">
                        from: {moment(event.start).format('ddd DD MMM YY LT')}
                      </label>
                      <label className="block text-black text-sm  mb-1 pt-1">
                        to: {moment(event.end).format('ddd DD MMM YY LT')}
                      </label>
                    </div>
                  </div>
